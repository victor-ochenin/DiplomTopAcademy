...
- `action` — асинхронная функция, которая принимает предыдущее состояние и данные формы
- `initialState` — начальное значение состояния
- `state` — текущее состояние (результат последнего выполнения action)
- `formAction` — обёрнутая функция для передачи в `action` пропс формы
- `isPending` — флаг, указывающий на выполнение action
